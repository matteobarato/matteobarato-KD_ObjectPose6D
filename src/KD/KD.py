...
class BaseClass:
    ...
    def _train_student(
        ...
        for ep in range(epochs):
            ...
            for (data, label) in self.train_loader:
                ...
                if not self.scheduler_student is None:
            ...
            print("Gates mean: ", [torch.mean(w[0].transformed_weight(), dim=0).item() for w in self.gate_layers])
    ...
    def save_checkpoint(self, net, acc, epoch, save_model_pth):
        state = {
            'net': net.state_dict(),
            'acc': acc,
            'epoch': epoch,
